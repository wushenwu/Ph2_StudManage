...
BOOL CServerApp::InitInstance()
{
    ...
    //DataBase Releated
    if (!pMainFrame->m_DataBase.ConnectDB())
    {
        return FALSE;
    }

    if (!pMainFrame->m_DataBase.CreateConnectionPool())
    {
        return FALSE;
    }
...
#if 0
    //UI Releated    
    CTblTreeView* pTblTreeView = reinterpret_cast<CTblTreeView*>(pMainFrame->m_SplitterWnd2.GetPane(0,0));
    if (pTblTreeView)
    {
        pTblTreeView->LoadTblTree();
    }
#endif
