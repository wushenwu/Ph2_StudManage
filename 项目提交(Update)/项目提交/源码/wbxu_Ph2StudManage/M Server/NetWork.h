// NetWork.h: interface for the CNetWork class.
//
//////////////////////////////////////////////////////////////////////

#if !defined(AFX_NETWORK_H__2D804AB6_434D_4E11_AA44_20689484B075__INCLUDED_)
#define AFX_NETWORK_H__2D804AB6_434D_4E11_AA44_20689484B075__INCLUDED_

#if _MSC_VER > 1000
#pragma once
#endif // _MSC_VER > 1000

#pragma warning(disable: 4786)

#include <afxtempl.h>
#include <map>
using namespace std;

#include "..\\MySocket\\MySocket.h"
#include "..\\MySocket\\MyPacket.h"
#include "..\\ThreadPool\\MyThreadPool.h"
#include "..\\ThreadPool\\IWorkItemBase.h"

/************************************************************************/
/* M Server层对交互数据的处理                                          */
/************************************************************************/
class CNetWork  
{
public:
    CNetWork();
    virtual ~CNetWork();

public:
    BOOL LoadServInfo();
    void OnStopService();
    void OnStartService();

    void ClassifyInfo(char *szBuff, 
                        sockaddr_in addrin
                        );   	
public:
    void ProcessSQLReq(char *pData, struct sockaddr_in addrin);
	BOOL ProcessUDPInit(char *pRequest, int nLen, struct sockaddr_in addrin);

	BOOL CheckLegal(char *pData);
	void GetColumns(char *pszTblNames);

protected:
    void SaveAlias(char *pszBuff);
    void ProcessSQLUpdate(char *pData, struct sockaddr_in addrin);
    void ProcessSQLDel(char *pData, struct sockaddr_in addrin);
    void ProcessSQLInsert(char *pData, struct sockaddr_in addrin);
	void ProcessSQLSelect(char *pData, struct sockaddr_in addrin);

public:
    CMySocket m_MySocket;
    CMyThreadPool m_ThreadPool;

private:
    CString m_strTblNames;
    CString m_strAlias;
    map<CString, CString> m_map_tbl_cols;
    map<CString, CString> m_map_ori_alias;  //原表、字段名 ---- 别名
    map<CString, CString> m_map_alias_ori;  //别名-----原表、字段名

private:
    //M Server
    CString m_strMIP;    
    unsigned int m_uhMPort;

    //Server
    CString m_strIP;
    unsigned int m_uhPort;
};

#endif // !defined(AFX_NETWORK_H__2D804AB6_434D_4E11_AA44_20689484B075__INCLUDED_)
