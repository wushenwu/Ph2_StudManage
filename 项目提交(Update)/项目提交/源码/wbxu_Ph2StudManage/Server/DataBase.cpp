...
void CDataBase::RefreshUpdateTime()
{
    ...
    //not a good idea to flush every time
}
...
{
    if (0 == m_strUpdateTime.GetLength())
    {
        m_strUpdateTime = "0";
    }
    ...
    strsql.Format("update t_alias set aliasname = '%s' where name = 'updatetime'", 
                                   m_strUpdateTime);
    try 
    {
        //exec the command
